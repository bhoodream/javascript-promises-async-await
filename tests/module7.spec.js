...
describe('Module 7', () => {
        ...
        if (checkFileExists('index')) {
            ...
            walk.simple(res, {
                FunctionDeclaration(node) {
                    if (
                        _.get(node, 'id.name', '') === 'getBooksAndMoviesAsync'
                    ) {
    ...
    it('should have a try/catch block in `getBooksAndMoviesAsync()` @try-catch-in-async-get-books-and-movies', () => {
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(catchWalk['MemberExpression'], 'object.name', '')
            ).to.equal(
                'console',
    ...
    it('should use Promise.all() to await the results @call-and-await-promise-all', () => {
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(tryBlock.MemberExpression, 'object.name', '')
            ).to.equal('Promise');
            expect(
                _.get(tryBlock.MemberExpression, 'property.name', '')
            ).to.equal('all');
    ...
    it('should return both books and movies as an object @return-books-and-movies', () => {
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(
                    tryBlock.ReturnStatement,
                    'argument.properties[0].key.name',
                    ''
                )
            ).to.be.oneOf(
                ['books', 'movies'],
            ...
            expect(
                _.get(
                    tryBlock.ReturnStatement,
                    'argument.properties[1].key.name',
                    ''
                )
            ).to.be.oneOf(
                ['books', 'movies'],
    ...
    it('should have async function `getBooksOrMoviesAsync()` @create-async-get-books-or-movies', () => {
        if (checkFileExists('index')) {
            ...
            walk.simple(res, {
                FunctionDeclaration(node) {
                    if (
                        _.get(node, 'id.name', '') === 'getBooksOrMoviesAsync'
                    ) {
                        localFunctions.push(node);
                    }
                },
    ...
    it('should use Promise.race() to await the results @call-promise-race', () => {
        if (checkFileExists('index')) {
            ...
            if (func.node) {
                ...
                expect(
                    _.get(tryBlock.VariableDeclarator, 'id.name', '')
                ).to.equal(
                    'values',
                ...
                expect(
                    _.get(
                        tryBlock.ArrayExpression,
                        'elements[0].callee.name',
                        ''
                    )
                ).to.equal(
                    'asyncFetchBooks',
    ...
    it('should return results of the race @promise-race-return-results', () => {
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(tryBlock.ReturnStatement, 'argument.name', '')
            ).to.equal('values', 'You should return the `values` variable.');
        }
    });
    ...
    it('should log the results of getBooksAndMovies @exec-getbooksandmovies-log', () => {
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(func.raceThenBody, 'callee.property.name', '')
            ).to.equal(
                'log',
            ...
            expect(
                _.get(
                    func.raceThenBody,
                    'arguments[1].properties[0].key.name',
                    ''
                )
            ).to.be.oneOf(
                ['movies', 'books'],
            ...
            expect(
                _.get(
                    func.raceThenBody,
                    'arguments[1].properties[1].key.name',
                    ''
                )
            ).to.be.oneOf(
                ['movies', 'books'],
