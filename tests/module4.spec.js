        ...
        if (checkFileExists('index')) {
            ...
            const moviePromiseExpression = res.body.filter(
                (node) =>
                    ...
                    _.get(node, 'expression.callee.object.name', '') ===
                        'moviePromise'
            )[0];
            expect(
            ...
            expect(
                _.get(
                    moviePromisevariableDeclaration,
            ...
            const fetchMoviesNode = res.body.filter(
                (node) =>
                    (node.type === 'ExportNamedDeclaration' &&
                        _.get(node, 'declaration.type', '') ===
                            'FunctionDeclaration' &&
                        _.get(node, 'declaration.id.name', '') ===
                            'fetchMovies') ||
                    (node.type === 'ExportNamedDeclaration' &&
                        _.get(node, 'declaration.type', '') ===
                            'VariableDeclaration' &&
    ...
    it('should use Promise.all to resolve both promises @get-books-and-movies-with-promise-all', () => {
        if (checkFileExists('index')) {
            ...
            if (varDec) {
                ...
                const body = _.get(
                    varDec,
                    'declarations[0].init.body.body[0]',
                    ''
                );
                expect(
                    _.get(promiseResultParams, 'elements[0].name', '')
                ).to.equal(
                    'books',
                    'You should pass `books` as the first array item in the parameter.'
                );
                expect(
                    _.get(promiseResultParams, 'elements[1].name', '')
                ).to.equal(
                    'movies',
                ...
                expect(
                    ...
                    'all',
                    'Use the `all()` method on the `Promise` object'
                );
            } else {
                const promiseResultParams = _.get(
                    funcDec,
        ...
        if (checkFileExists('index')) {
            ...
            expect(
                _.get(
                    ...
                    'declarations[0].init.callee.name',
                    ''
                )
            ).to.equal(
                'getBooksAndMovies',
            ...
            expect(
                ...
                'then',
                'You need to resolve the promise using `.then()`'
            );
            expect(
                _.get(
                    getBooksAndMoviesResults,
        ...
        if (checkFileExists('index')) {
            ...
            if (varDec) {
                ...
                expect(
                    ...
                    'race',
                    'Use the `race()` method on the `Promise` object'
                );
            } else {
                const promiseResultParams = _.get(
                    funcDec,
                ...
                expect(
                    ...
                    'race',
                    'Use the `reace()` method on the `Promise` object'
                );
                expect(_.get(promiseResultParams, 'name', '')).to.equal(
                    'results',
                    'You should pass `results` as the first array item in the parameter.'
